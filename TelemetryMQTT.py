...
async def print_all_info(drone):
    while True:
        ...
        roll, pitch, heading = await get_bodyangles(drone)

        print(f"{round(lat, 6)} {round(long, 7)} {round(alt, 1)} {round(Vnorth, 1)} {round(Veast, 1)} {round(Vdown, 1)} {round(heading, 1)} {round(roll, 1)} {round(pitch, 1)}")
...
async def Send_MQTT_Data(drone):
    while True:
        ...
        telem = f"{round(lat, 6)} {round(long, 7)} {round(alt, 1)} {round(Vnorth, 1)} {round(Veast, 1)} {round(Vdown, 1)} {round(heading, 1)} {round(roll, 1)} {round(pitch, 1)}"
        message = re.sub(r'\.', ',', telem)

        publish.single(topic, str(message), hostname=hostname, port=port)
        print(message)
        #publish.single("Detections/", str(message), hostname=hostname, port=port)
...
async def get_bodyangles(drone):
    async for attitude_data in drone.telemetry.attitude_euler():
        return attitude_data.roll_deg, attitude_data.pitch_deg, attitude_data.yaw_deg
